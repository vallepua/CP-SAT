...
def max_equal_k_by_cpsat(N: int, C: int,
    ...
    while lo <= hi:
        mid = (lo + hi) // 2
        board, status, rt = solve_knights_equal_k(N, C, mid,
...
def render_board(board: List[List[int]], title: str = "") -> plt.Figure:
    ...
    # palette
    armies_present = sorted(
        set(v for row in board for v in row if v >= 0)
    )
...
with left:
    ...
    run = st.button("Compute maximum equal k using CP-SAT",
                    type="primary",
...
with right:
    ...
    if run:
        ...
        if board is not None:
            # ⬇️ updated title: use n, c, no k*
            fig = render_board(board, title=f"n={N}, c={C}")
...
if go_sweep:
    ...
    try:
        ...
        st.download_button(
            "Download CSV",
            data=csv,
            file_name="peaceable_knights_results.csv",
            mime="text/csv"
        )
    except Exception:
        st.text(rows)
